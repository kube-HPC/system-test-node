...
const randomize = (webhook="") => {
    const nodesNum = Math.floor(Math.random() * 9) + 7
    ...
    if(webhook != ""){
        desciptor.webhook =  {
        "progress": `http://${webhook}/progress/random`,
        "result": `http://${webhook}/results/random`
        }
    }
    desciptor.name = "randompipe"+nodesNum
    return desciptor
}
